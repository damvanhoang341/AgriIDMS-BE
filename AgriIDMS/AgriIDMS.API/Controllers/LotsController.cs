...
namespace AgriIDMS.API.Controllers
{
    ...
    public class LotsController : ControllerBase
    {
        ...
        {
            var overrides = await _lotService.GetProductVariantDiscountOverridesAsync();
            return Ok(overrides);
        }

        [HttpPut("product-variant-discount-overrides")]
        [Authorize(Roles = "Admin,Manager")]
        public async Task<IActionResult> UpdateProductVariantDiscountOverrides([FromBody] List<UpsertProductVariantDiscountOverrideDto> overrides)
        {
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier) ?? string.Empty;
            await _lotService.UpdateProductVariantDiscountOverridesAsync(userId, overrides ?? new List<UpsertProductVariantDiscountOverrideDto>());
            return Ok(new { message = "Đã cập nhật cấu hình override giảm giá theo sản phẩm." });
        }
    }
}
