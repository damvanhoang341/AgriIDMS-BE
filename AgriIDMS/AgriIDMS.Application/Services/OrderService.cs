...
namespace AgriIDMS.Application.Services
{
    public class OrderService : IOrderService
    {
            ...
            ILogger<OrderService> logger)
        {
            ...
            _onlineOrderSoftLockDuration = TimeSpan.FromMinutes(softLockMinutes);
            _logger = logger;
        }
        ...
        public async Task<AllocationProposalOverviewDto> GetAllocationProposalsAsync(int orderId)
        {
                ...
                && order.Status != OrderStatus.Confirmed)
            {
                throw new InvalidBusinessRuleException(
                    $"Chỉ xem proposal khi đơn đang chờ kho xác nhận hoặc đã Confirmed (xem thùng đã giữ). Hiện tại: {order.Status}");
            }
            ...
            var detailSummaries = order.Details.Select(d =>
            {
                ...
                var requestedQty = (int)d.Quantity;
        ...
        {
            if (paymentTiming != PaymentTiming.PayBefore && paymentTiming != PaymentTiming.PayAfter)
                throw new InvalidBusinessRuleException("Chỉ được chọn PayBefore (trả trước) hoặc PayAfter (trả sau).");
            ...
            if (order.Source != OrderSource.Online)
                throw new InvalidBusinessRuleException("Chỉ đơn online mới chọn hình thức thanh toán qua bước này.");
            ...
            return await GetMyOrderByIdAsync(orderId, userId);
        }
            ...
            await _uow.ExecuteInRetryableTransactionAsync(async () =>
            {
                ...
                if (allocations != null && allocations.Any())
                {
                    foreach (var alloc in allocations)
                    {
                        ...
                        else if (alloc.Status == AllocationStatus.Proposed ||
                                 alloc.Status == AllocationStatus.SoftLocked)
                        {
                            alloc.Status = AllocationStatus.Cancelled;
                        }
                    }
        ...
        public async Task<CreateOrderFromCartResponse> CreatePosOrderAsync(string operatorUserId, CreatePosOrderRequest request)
        {
            ...
            await _uow.ExecuteInRetryableTransactionAsync(async () =>
            {
                ...
                var order = new Order
                {
                    ...
                    // POS: giữ đủ thùng ngay khi tạo (giống online); không qua hàng chờ allocate / thiếu hàng.
                ...
                await ReserveStockImmediatelyForPosAsync(order, now);
                ...
                response = new CreateOrderFromCartResponse
                {
                    ...
                    AllocationSucceeded = true,
                    AllocationMessage =
                        $"Đã giữ (Reserved) đủ thùng cho đơn POS — hết hạn giữ theo cấu hình kho ({AllocationExpirationHours} giờ).",
        ...
        public async Task<ConfirmAllocationResultDto> ConfirmAllocationAsync(int orderId, string operatorUserId, bool skipCustomerOwnershipCheck = false)
        {
            ...
            if (!IsInitialAllocationStatus(order.Status))
                throw new InvalidBusinessRuleException(
                    $"Chỉ xác nhận allocate khi đơn đang chờ giữ hàng / chờ kho xác nhận. Hiện tại: {order.Status}");
            ...
            await _uow.ExecuteInRetryableTransactionAsync(async () =>
            {
                ...
                foreach (var detail in order.Details)
                {
                    ...
                    detail.ShortageQuantity = Math.Max(0, boxesNeeded - allocated);
                    totalAmount += allocated * detail.BoxWeight * detail.UnitPrice;
                }

                if (!IsOrderFullyReserved(order))
                    throw new InvalidBusinessRuleException(
                        "Không đủ thùng để allocate đủ đơn (một phần thùng trong đề xuất không còn khả dụng). Vui lòng chạy lại đề xuất allocate (re-propose) rồi xác nhận.");
        ...
        private async Task ValidateAllocationRequestAsync(Order order, string operatorUserId, bool skipCustomerOwnershipCheck)
        {
            ...
            if (!canAllocate)
                throw new InvalidBusinessRuleException(
                    $"Chỉ có thể giữ hàng khi đơn đang chờ giữ hàng / chờ kho xác nhận. Hiện tại: {order.Status}");
        ...
        private async Task FinalizeSaleConfirmForReservedOnlineOrderAsync(Order order, DateTime utcNow)
        {
            ...
            var saleConfirmWindowExpired = allAllocations.Any(a =>
                (a.Status == AllocationStatus.Reserved || a.Status == AllocationStatus.SoftLocked) &&
        ...
        private async Task<List<OrderAllocation>> ReserveBoxesForNewOnlineOrderAsync(Order order, DateTime nowUtc)
        {
            ...
            var expiresAt = nowUtc.Add(_onlineOrderSoftLockDuration);
        ...
        /// <summary>POS TakeAway &amp; Delivery: FEFO + tồn offline, Reserved ngay, đủ số lượng hoặc throw.</summary>
        private async Task ReserveStockImmediatelyForPosAsync(Order order, DateTime nowUtc)
        {
            foreach (var detail in order.Details)
            {
                ...
                if (selectedBoxes.Count < neededBoxes)
                {
                    var variantLabel = await GetVariantDisplayLabelAsync(detail.ProductVariantId, detail.ProductVariant);
                    throw new InvalidBusinessRuleException(
                        $"Không đủ thùng khả dụng để giữ hàng ngay tại quầy cho {variantLabel}. " +
                        $"Thiếu {neededBoxes - selectedBoxes.Count} thùng, " +
                        $"quy cách {(detail.IsPartial ? "thùng lẻ" : "thùng đầy")} {detail.BoxWeight:N2} kg/thùng.");
                }
        ...
        private async Task<string> GetVariantDisplayLabelAsync(int productVariantId, ProductVariant? variant)
        {
            var loaded = variant ?? await _variantRepo.GetProductVariantByIdAsync(productVariantId);
            if (loaded == null)
                return $"biến thể #{productVariantId}";

            var productName = loaded.Product?.Name?.Trim();
            var grade = loaded.Grade.ToString();
            if (string.IsNullOrWhiteSpace(productName))
                return $"biến thể {grade}";

            return $"sản phẩm {productName} - loại {grade}";
        }
